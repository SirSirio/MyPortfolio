...
- **Tech stack**: **Canvas-2D "cosmic engine"** (`star-engine.js`) plus modern HTML/CSS/JS, per the approved Claude Design ("Sirio V4 - Deep Field × Hyperlight") — this **supersedes the earlier Full WebGL / Three.js intent** (Canvas-2D lands the wow and stays lighter on mobile). The hard constraint is **static + deployable to GitHub Pages (no server runtime)**, *not* "vanilla only"; **selective modern libraries are welcome where they earn their place** — Phase 2.1 adds **GSAP + ScrollTrigger** (vendored) for project-page scroll-telling (Phase 2.1 D-17–D-19). *Why:* maximum wow ceiling with mobile-lightness; must still build to a static bundle deployable to GitHub Pages. *(NOTE: the "Key Decisions" table below and the hero-goal bullet still describe the pre-pivot WebGL engine — historical record, superseded by this line and by 05_Portfolio/CLAUDE.md.)*
