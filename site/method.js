...
export function initMethod() {
  ...
  /* Reduced motion: resolve immediately and DO NOT attach an observer. Under
     REDUCED there is no sequence to play, so a missed or late IO callback would
     leave the calculator dimmed and unresolved forever (the sketch learned this
     the hard way). */
  if (REDUCED) {
    playCalc();
    return;
  ...
  /* Play on the calculator's own scroll position, once. threshold 0.18 is the
